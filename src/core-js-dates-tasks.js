...
function getNextFriday(date) {
  const TARGET_WEEKDAY_NUMBER = 5;
  const WEEKDAYS_AMOUNT = 7;

  const currentDayNumber = getWeekdayNumber(date);

  const isBeforeFriday = currentDayNumber < TARGET_WEEKDAY_NUMBER;

  const daysUntilFriday = isBeforeFriday
    ? TARGET_WEEKDAY_NUMBER - currentDayNumber
    : TARGET_WEEKDAY_NUMBER + WEEKDAYS_AMOUNT - currentDayNumber;

  const daysBeforeNextFriday = date.getDate() + daysUntilFriday;
  const nextFriday = new Date(date);
  nextFriday.setDate(daysBeforeNextFriday);
  return nextFriday;
}
...
const getDaysFromMilliseconds = (milliseconds) =>
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return getDaysFromMilliseconds(difference) + 1;
}
...
function isDateInPeriod(date, { start, end }) {
  ...
  return dateString >= new Date(start) && dateString <= new Date(end);
}
...
function getWeekNumberByDate(date) {
  ...
  const january1stWeekday = january1st.getDay() || DAYS_PER_WEEK;
  const diff =
    january1stWeekday <= 4 ? 0 : DAYS_PER_WEEK - january1stWeekday + STEP;
  const firstWeekStart = new Date(january1st + diff);
  return Math.ceil(
    ((date - firstWeekStart) / MS_PER_DAY + STEP) / DAYS_PER_WEEK
  );
}
...
function getNextFridayThe13th(date) {
  const MS_PER_DAY = 86400000;
  const day = new Date(date.getTime() + MS_PER_DAY).getUTCDate();

  const isStartThisMonth = day <= 13;

  const month = date.getMonth();
  const year = date.getFullYear();

  let startMonth = isStartThisMonth ? month : month + 1;

  let day13th = new Date(year, startMonth, 13);
  while (day13th.getDay() !== 5) {
    startMonth += 1;
    day13th = new Date(year, startMonth, 13);
  }
...
const parseStringDate = (string) => {
  const [day, month, year] = string.split('-');
  return new Date(year, month - 1, day);
};

function getWorkSchedule({ start, end }, countWorkDays, countOffDays) {
  const startDate = parseStringDate(start);
  const endDate = parseStringDate(end);
  const schedule = [];
